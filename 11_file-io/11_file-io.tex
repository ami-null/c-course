...
\begin{document}
    ...
    \begin{frame}{FILE Pointer}
        \begin{itemize}
            \item Files are accessed using file pointers: \texttt{FILE *}
            \item A \texttt{FILE *} variable does not hold actual file content
    ...
    \begin{frame}{Life Cycle of using a File}
        \begin{enumerate}
            \item Open the file using \texttt{fopen()}
            \item Read from or write to the file
            \item Close the file using \texttt{fclose()}
        \end{enumerate}
    ...
    \begin{frame}[fragile]{Opening Files}
        To work with files, we must first open them using \texttt{fopen()}.
...
FILE *fp = fopen("filename.txt", "mode");
        ...
        Basic modes:
        \begin{itemize}
            \item \texttt{"r"}: open for reading; fails if file does not exist
            \item \texttt{"w"}: open for writing; creates new file or overwrites existing file
            \item \texttt{"a"}: open for appending (adding new contents to existing file); creates file if needed
        \end{itemize}
    \end{frame}
...
if(fp == NULL){
    // error
        ...
        \begin{itemize}
            \item File does not exist (when using \texttt{"r"})
    ...
    \begin{frame}{Closing Files}
        Closing files is done using \texttt{fclose(fp);}.
    ...
    \begin{frame}[fragile]{Writing to a File}
        ...
        Common functions:
        \begin{itemize}
            \item \texttt{fprintf()}: write formatted text.
            \item \texttt{fputs()}: write a string.
            \item \texttt{fputc()}: write a single character.
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}[fragile]{Reading from a File}
        ...
        Functions:
        \begin{itemize}
            \item \texttt{fscanf()}: formatted reading
            \item \texttt{fgets()}: read a line
            \item \texttt{fgetc()}: read a single character
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Basic Error Handling}
        ...
        \begin{itemize}
            \item \textbf{Failed fopen}: always check if pointer returned is \texttt{NULL}
            \item \textbf{Reaching EOF}: functions return special values (e.g., \texttt{NULL} for \texttt{fgets}, \texttt{EOF} for \texttt{fgetc})
            \item \textbf{Read errors}: typically uncommon in small programs but detectable using \texttt{ferror()}
        \end{itemize}
        ...
        In basic programs, it is usually enough to check for \texttt{NULL} or \texttt{EOF}
    \end{frame}


    \begin{frame}{Summary}
        \begin{itemize}
            \item Use \texttt{fopen()} to open files with the correct mode
            \item Perform reading or writing using \texttt{fprintf()}, \texttt{fputs()}, \texttt{fgetc()}, \texttt{fgets()}, or \texttt{fscanf()}
            \item Always close files using \texttt{fclose()}
