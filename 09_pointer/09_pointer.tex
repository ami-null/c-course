...
\begin{document}
    ...
    \begin{frame}[fragile]{Defining a Pointer}
        Pointers are defined by putting a \textttbg{*} before the pointer's name:
        \begin{minted}{c}
int x = 10;
...
scanf("%d", p);    // takes user input into x
        ...
        In the last line, the \textttbg{\&} sign is not needed because \textttbg{p} holds the memory address of \textttbg{x}.
    \end{frame}


    \begin{frame}[fragile]{Pointers and Memory Addresses}
        \begin{minted}{c}
int x = 10, *px = &x;
        \end{minted}
...
printf("%d", *px);
        ...
        \vspace{1em}
        This is due to \textttbg{px} holding the memory address of \textttbg{x}.
    \end{frame}
    ...
    \begin{frame}[fragile]{Printing an Array}
        \begin{minted}{c}
int arr[] = {1, 2, 3, 4, 5}
        \end{minted}
...
\section{Pointers with Function}
    ...
    \begin{frame}[fragile]{Example: Array Passed to Function}
        ...
        In line 3, \textttbg{int arr[]} can be replaced with \textttbg{int *arr}. They are equivalent.
    \end{frame}
