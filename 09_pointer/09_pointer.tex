...
scanf("%d", p);    // takes user input into x
        ...
        In the last line, the \texttt{\&} sign is not needed because \texttt{p} holds the memory address of \texttt{x}.
    \end{frame}
...
printf("%d", *px);
        ...
        \vspace{1em}
        This is due to \texttt{px} holding the memory address of \texttt{x}.
    \end{frame}
...
\section{Pointers with Function}
    ...
    \begin{frame}{Example: Modify Array in Function (revisited)}
        \inputminted[linenos]{C}{../code-examples/09_07_function-modify-array_2.c}
    \end{frame}
