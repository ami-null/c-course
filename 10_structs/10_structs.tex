...
\begin{document}
    ...
    \begin{frame}[fragile]{Defining a Structure}
        Below a new type layout called \textttbg{struct Student} is created:
        \begin{minted}{c}
struct Student {
    char name[50];
    int id;
    float cgpa;
        ...
        \begin{itemize}
            \item Structs are usually created outside and above the main function, so that all functions inside the \textttbg{C} file can access the struct
    ...
    \begin{frame}[fragile]{Declaring Structure Variables}
        Three variable of the type \textttbg{struct Student} have been defined below:
        \begin{minted}{c}
struct Student s;
...
struct Person {
    ...
    \section{Defining Types with \textttbg{typedef}}

    \begin{frame}{The \texttt{typedef} Keyword}
        \begin{itemize}
            \item A structure type name often becomes long and repetitive
            \item Using \textttbg{typedef} helps create cleaner and shorter type names
            \item Without typedef: \textttbg{struct Student s;}
            \item With typedef: \textttbg{Student s;}
        \end{itemize}
...
p->cgpa = 3.70;    // pointer access
        \end{minted}
        In the last line, we are setting the \textttbg{cgpa} field to 3.70. This \textttbg{cgpa} field belongs to the struct whose memory address is stored in \textttbg{p}.
    \end{frame}
...
void print_student(const struct Student *s);
    ...
    \begin{frame}{Exercises}
        \begin{enumerate}
            \item Define a struct to hold the different parts (city, road name, house number etc.) of the address of a person. Declare a variable of this struct type, set values to its elements by taking user input, then print the values
