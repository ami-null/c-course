...
\section{Introduction to Arrays}
    ...
    \begin{frame}{What is an Array?}
        \begin{itemize}
            ...
            \item Each element is accessed by an \textbf{index} (zero-based in C, meaning that the first element is at index 0): \textttbg{arr[0]}, \textttbg{arr[1]}, \dots
...
int b[]  = {1, 2, 3};    // size inferred: 3
    ...
    \begin{frame}{Accessing Individual Elements}
        \begin{itemize}
            \item Use square brackets with index: \textttbg{arr[index]}
            \item Example: \textttbg{arr[2]} accesses the third element
        \end{itemize}
    ...
    \begin{frame}{Multidimensional Array}
        \begin{itemize}
            ...
            \item Real-life analogies:
                \begin{itemize}
                    \item 2D: matrix or spreadsheet (rows and columns), indices are written as \textttbg{arr[row][col]}
...
int mat3[2][3] = {1,2,3,4,5,6};
    ...
    \begin{frame}{Accessing Values in 2D Arrays}
        \begin{itemize}
            \item Access element at row r, column c by \textttbg{mat[r][c]}
            \item Example: \textttbg{mat[1][2]} refers to second row, third column
        \end{itemize}
    ...
    \begin{frame}{3D and Higher Dimensions}
        \begin{itemize}
            \item A 3D array \textttbg{int arr[2][3][4];} can be thought of as 2 blocks, each block is a 3x4 matrix
            \item Real-life: for example, \textbf{block × row × column} measurements (temperature map over multiple days)
            \item Indexing: \textttbg{a[block][row][col]}
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{What is a String in C?}
        \begin{itemize}
            \item In C, a \textbf{string} is an array of \textttbg{char} terminated by the null character \textttbg{'\textbackslash0'}
            \item Example: \textttbg{char s[] = "hello";} actually creates 6 chars: \textttbg{'h','e','l','l','o','\textbackslash0'}
            \item Strings are manipulated through arrays and standard library functions in \textttbg{<string.h>}
            \item You can access individual characters with \textttbg{s[i]}
        \end{itemize}
    \end{frame}
...
Note: individual characters denoted by single quotes, whereas strings are denoted by double quotes.
    ...
    \begin{frame}[fragile]{Reading Strings from User}
        \begin{itemize}
            \item Avoid \textttbg{gets()} (unsafe). Use \textttbg{fgets()} or \textttbg{scanf("\%s", ...)}
            \item \textttbg{scanf("\%s", s);} reads until whitespace, does not read spaces
            \item \textttbg{fgets(s, size, stdin);} reads a whole line (including spaces)
            \item However, the string from \textttbg{fgets()} includes newline (\verb|'\n'|)
    ...
    \begin{frame}{Example: String Input Using \texttt{scanf()}}
        ...
        Note that in \textttbg{scanf()}, we provided \textttbg{username} as the second argument and not \textttbg{\&username}.
        
        This is because \textttbg{username} itself holds the memory address of the character array.

        Recall that the \textttbg{\&var} returns the memory address of the variable named \textttbg{var}.
    \end{frame}
    ...
    \begin{frame}{Example: String Input Using \texttt{fgets()} (cont.)}
        ...
        In the above code, \textttbg{strcspn(s, "\textbackslash n")} searches for and returns the index of the newline character (\textttbg{"\textbackslash n"}) in the string named \textttbg{s}.\\

        \textttbg{s[strcspn(s, "\textbackslash n")] = '\textbackslash 0';} replaces the newline character with the null terminator character (\textttbg{'\textbackslash 0'}).
    \end{frame}


    \begin{frame}{Common String Functions (from <string.h>)}
        \begin{itemize}
            \item \textttbg{strlen(s)} — length of string (not counting \textttbg{'\textbackslash0'})
            \item \textttbg{strcmp(s1, s2)} — compare strings (returns 0 if equal)
            \item \textttbg{strcpy(dest, src)} — copy string
            \item \textttbg{strcat(dest, src)} — concatenate
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Example: Comparing Two Strings: \texttt{strcmp()}}
        ...
        \textttbg{strcmp()} returns 0 if the two strings are same.
    \end{frame}
    ...
    \begin{frame}{Summary}
        \begin{itemize}
            \item \textbf{Array:} contiguous collection of same-type elements, accessed by indices \textttbg{arr[i]}
            \item \textbf{1D/2D/3D:} use \textttbg{arr[i]}, \textttbg{arr[i][j]}, \textttbg{arr[i][j][k]} respectively
            \item \textbf{Strings:} arrays of \textttbg{char} ending with \textttbg{'\textbackslash0'}
            \item \textttbg{<string.h>} contains functions that operate on strings
            \item \textbf{Input:} \textttbg{scanf} or \textttbg{fgets} (preferred for whole lines)
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Array Variable and Memory}
        \begin{itemize}
            \item \textbf{Important note:} In many contexts (for example, when passing to a function), the array name (say, \textttbg{arr}) is a pointer to the first element (details on pointers in upcoming lectures)
            \item Meaning that, \textttbg{arr} actually holds the memory address in which the first element of the array is stored
    ...
    \begin{frame}[allowframebreaks=0.75]{Exercises}
        \begin{enumerate}
            \item Write a program to read \textttbg{n} integers into an array and print them in reverse order
            ...
            \item Write a program to rotate the elements of an array to the right by \textttbg{k} positions
            \item Write a program to read a line of text and print its length (without using \textttbg{strlen})
            \item Write a program to concatenate two strings without using \textttbg{strcat}
